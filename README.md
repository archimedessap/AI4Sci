...
补抓默认读取：
- `scripts/ai4sci_sources.txt`（重要期刊/会议）
...
提示：
- 支持 `--keywords-file` / `--sources-file` / `--ai-keywords-file`（每行一个）
...
用 `cron`/GitHub Actions/服务器定时器周期性运行 `python3 scripts/update_progress_openalex.py`，网站会在下一次请求时读到最新 `base.json`（首页已设置为动态渲染）。

GitHub Actions 示例见 `/.github/workflows/daily-update.yml`（每日跑 `python3 scripts/update_all.py --openalex-full --daily-updates`）。如需 LLM 分类，请在仓库 Secrets 配置 `DEEPSEEK_API_KEY`（默认使用 DeepSeek；可用 `LLM_PROVIDER` 切换）。
